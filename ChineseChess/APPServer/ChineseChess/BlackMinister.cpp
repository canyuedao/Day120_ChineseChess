...
CBlackMinister::CBlackMinister(char *cColor,int x,CChessBoard *pChessBoard)
{
...
CBlackMinister::~CBlackMinister(void)
{
	delete []mAllAccess;
	delete []mCurAccess;
	delete mPosition;
...
void CBlackMinister::FindCurAccess(char *cColor){
	int x=mPosition->x;
	int y=mPosition->y;
	int iindex=0;
		...
		if(y+2<5&&x+2<9){	
			if(!(mChessBoard->GetPosition(x+1,y+1)->mPChess)){
				if(!(mChessBoard->GetPosition(x+2,y+2)->mPChess)){
					...
					if(mChessBoard->GetPosition(x+2,y+2)->mPChess->mColor=="Red"){
						mCurAccess[iindex]=*(mChessBoard->GetPosition(x+2,y+2));
						iindex++;
					}
				}				
...
void CBlackMinister::FindAllAccess(char *cColor){	
	if(cColor=="Black"){
